...
init 10 python:
    def switch_Textbox_visibility():
        click = 1 if persistent.TextBoxVisible else 2 

        if click == 1:
            persistent.TextBoxVisible = False
            click = 2
        ...
        if click == 3:
            click = 1
            persistent.TextBoxVisible = True
            renpy.restart_interaction()
            return
...
label splashscreen_jgs:
    ...
    if not steam_build:
        ...
        if persistent.SplashUnlocked:
            scene pre_game_01
        elif True:
            scene pre_game_02
        show screen start_buttons()
        $ ui.interact()
    elif True:
        show screen in_animation with dissolve
        show screen start_AForeignWorld with Dissolve(1)
        pause(1)
        hide screen start_AForeignWorld with Dissolve(1)
        show intro_01 with Dissolve(.2)
        pause(1.9)
        hide intro_01 with Dissolve(.5)
        pause(1)
