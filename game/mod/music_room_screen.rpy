screen musicroom():
    ...
    if not main_menu:
        on "replace" action [SetLocalVariable("current_track", mr.get_track()),
            SetLocalVariable("current_loop", renpy.music.get_loop()), SetLocalVariable("trigger", 1)]
        on "show" action [SetLocalVariable("current_track", mr.get_track()),
            SetLocalVariable("current_loop", renpy.music.get_loop()), SetLocalVariable("trigger", 2)]
        if current_track:
            if not mr.get_track() == current_track:
                key ("K_m", "game_menu") action Play("music", current_loop, loop=True if current_loop else False), Return()
            else:
                key ("K_m", "game_menu") action Queue("music", current_loop, loop=True if current_loop else False), Return()
        else:
    ...
    use game_menu(_("Music".capitalize()), scroll=None):
        vbox:
            hbox:
                spacing 10
                xfill True
